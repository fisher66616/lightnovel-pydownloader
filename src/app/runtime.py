...
async def _task_runner(cancel_event: Optional[Event] = None):
    await Process(cancel_event=cancel_event).run()
...
async def run_async(
    ...
    try:
        if scheduler_enabled:
            ...
            await _task_runner(cancel_event=cancel_event)
...
def run_sync(
    ...
    try:
        return asyncio.run(
            run_async(enable_scheduler=enable_scheduler, cancel_event=cancel_event)
        )
    except TaskCancelled:
        raise
